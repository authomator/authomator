...
function Redirector() {
    ...
    this.createPasswordResetToken = function(user, done){

        // reduce validity time for reset and add action to audience
        var opts = _.merge(
            {}, 
            config.jwt.refresh.options, 
            {
                expiresInMinutes: config.reset.expiresInMinutes,
                audience: config.jwt.refresh.options.audience + '::password-reset'
            }
        );
...
Redirector.prototype.validatePasswordResetToken = function(token, callback) {

    //  add action to audience
    var opts = _.merge(
        {},
        config.jwt.refresh.options,
        {
